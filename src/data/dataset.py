...
class STEDataset:
    ...
    def create_splits(
        ...
        for api_name, examples in api_to_examples.items():
            ...
            val_samples = examples_copy[
                samples_per_api_train : samples_per_api_train + samples_per_api_val
            ]
        ...
        print(
            f"Created splits: demo={len(demo_set)}, train={len(train_set)}, "
    ...
    def create_leave_one_api_out_splits(
        self,
        held_out_api: str,
        demo_size: int = 4520,
        samples_per_api_train: int = 30,
        ...
        Paper Section 5 (Zero-Shot Generalization):
        "To test MICE's out-of-domain generalization, we simulate encountering
        new APIs by holding one out during training. Since there are 50 APIs
        present in the STE dataset, we train 50 MICE RF and 50 MICE LR models.
        Each model is trained on data from 49 APIs and evaluated solely on the
        held-out API."

        Args:
            held_out_api: API name to hold out from training
            demo_size: Size of demonstration set for ICL
            samples_per_api_train: Training samples per API (paper: 30)
            samples_per_api_val: Validation samples per API (paper: 15)

        Returns:
            demo_set: Examples for few-shot prompting (from 49 APIs)
            train_set: Training examples for MICE (from 49 APIs, 30 per API)
            val_set: Validation examples (from 49 APIs, 15 per API)
        ...
        if held_out_api not in api_to_examples:
            raise ValueError(f"API '{held_out_api}' not found in training data")

        # Sample stratified splits (excluding held-out API from train/val/demo)
        train_set = []
        val_set = []
        demo_pool = []

        for api_name, examples in api_to_examples.items():
            if api_name == held_out_api:
                # Skip this API - it will only be in test set
                continue
        ...
        # Test set contains ONLY the held-out API
        test_set = []
        if held_out_api in self.test_raw:
            for item in self.test_raw[held_out_api]:
                test_set.append(self._parse_example(item))
